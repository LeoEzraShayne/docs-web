...
const req: DocumentFieldConfig[] = [
  { key: "problems", label: "解決したい課題", required: true, sources: ["PROJECT", "DIRECT_INPUT"] },
  { key: "goals", label: "実現したいこと", required: true, sources: ["PROJECT", "DIRECT_INPUT"] },
  { key: "mainFunctions", label: "主な機能", required: true, sources: ["PROJECT", "DIRECT_INPUT"] },
  { key: "userTypes", label: "利用者", sources: ["PROJECT", "DIRECT_INPUT"] },
  { key: "externalSystems", label: "外部連携システム", sources: ["PROJECT", "DIRECT_INPUT"] },
  { key: "specialRequirements", label: "特別な要件", sources: ["PROJECT", "DIRECT_INPUT"] },
];
...
export const DOCUMENT_PAGE_CONFIGS: Record<DocumentType, DocumentPageConfig> = {
  REQUIREMENTS: {
    ...
    fields: req,
    sheets: ["項目概要", "スコープ定義", "業務要件", "機能要件一覧", "画面一覧", "画面概要", "権限一覧", "データ項目定義", "外部連携/API一覧", "非機能要件", "業務フロー", "課題・リスク一覧"],
  ...
  BASIC_DESIGN: {
    type: "BASIC_DESIGN",
    title: "基本設計書",
    sourceOptions: [{ label: "要件定義書", value: "REQUIREMENTS_VERSION" }, { label: "直接入力", value: "DIRECT_INPUT" }],
    ...
    sheets: ["基本設計概要", "システム構成設計", "画面遷移一覧", "画面設計", "機能設計", "API設計", "データベース設計", "権限設計", "バッチ・帳票設計", "非機能設計"],
  ...
  DETAILED_DESIGN: {
    ...
    title: "詳細設計書",
    sourceOptions: [{ label: "基本設計書", value: "BASIC_DESIGN_VERSION" }, { label: "直接入力", value: "DIRECT_INPUT" }],
    modes: ["standard", "custom"],
    fields: [
    ...
    sheets: ["テーブル詳細設計", "テーブル項目定義", "API項目定義", "API詳細設計", "画面項目設計", "入力チェック設計", "画面処理設計", "エラー設計", "メッセージ設計"],
  ...
  UNIT_TEST: {
    type: "UNIT_TEST",
    title: "単体テスト仕様書",
    sourceOptions: [{ label: "詳細設計書", value: "DETAILED_DESIGN_VERSION" }, { label: "直接入力", value: "DIRECT_INPUT" }],
    modes: ["standard", "custom"],
    fields: [
      { key: "screens", label: "画面一覧", required: true, sources: ["DIRECT_INPUT"] },
      { key: "apis", label: "API一覧", sources: ["DIRECT_INPUT"] },
      { key: "tables", label: "テーブル一覧", sources: ["DIRECT_INPUT"] },
      { key: "testViewpoints", label: "テスト観点", kind: "checkboxes", options: viewpoints },
  ...
  INTEGRATION_TEST: {
    ...
    title: "結合テスト仕様書",
    sourceOptions: [{ label: "詳細設計書", value: "DETAILED_DESIGN_VERSION" }, { label: "設計資料貼付", value: "PASTED_DESIGN" }],
    modes: ["standard", "custom"],
    fields: [{ key: "designMaterial", label: "設計資料", required: true, maxLength: 10000, sources: ["PASTED_DESIGN"] }],
    sheets: ["業務シナリオテスト"],
